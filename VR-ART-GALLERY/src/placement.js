...
module.exports = (regl, {placements, getAreaIndex}) => {
    ...
    const loadPainting = (p) => {
        ...
        let globalScale = (4.5 / (3 + p.aspect)) * 2.00; //tweaked to look good
        globalScale = Math.min(globalScale, segLen / p.aspect / 2.0); //clamp horizontal
        globalScale = Math.min(globalScale, 2 / 1.1); //clamp vertical
        const pos = [(seg[0][0] + seg[1][0]) / 2, 2.7 - globalScale, (seg[0][1] + seg[1][1]) / 2];
        ...
        const textmodel = [];
       mat4.fromTranslation(textmodel, [pos[0], 2.3 - globalScale, pos[2]]);
